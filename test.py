...
if __name__ == '__main__':
    ...
    train_transforms = T.Compose([T.Resize(448), T.CenterCrop(448), T.RandomHorizontalFlip(p=0.4), T.PILToTensor()])
    dataset_train = CUBDatasetSimple(os.path.join('datasets', 'CUB'), split='train', transforms=train_transforms)
    dataloader_train = DataLoader(dataset=dataset_train, batch_size=4, shuffle=True, num_workers=8)
